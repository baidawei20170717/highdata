<template>
<div>
  <div class="modal-dialog in index-dialog" style='display:none'>
            ...
            <table class="table table-hover">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    <div>{{node.noData ? '最近15分钟无数据' : '正常'}}</div>
                  </td>
                  <td>
                    <div>{{node.warn}}</div>
                  </td>
                  <td>
                    <div ng-if="node.cpu">{{node.cpu * 100 | number(2)}}%</div>
                  </td>
                  <td>
                    <div ng-if="node.memory">{{node.memory * 100 | number(2)}}%</div>
                  </td>
                </tr>
              </tbody>
            </table>
  ...
  <div class="index_dialog_bg" style="display:none"></div>
</div>
</template>
<script type="es6">
  export default {
    ...
    watch:{
      'node.name':function(val,oldVal){
        console.log(val)
        $('.index-dialog').show();
        $('.index_dialog_bg').show();
        $('.index_dialog_bg').click(function(){
          $('.index-dialog').hide();
          $(".index_dialog_bg").hide()
        })
      }
...
.index_dialog_bg{content:'';position:fixed;z-index: 1040;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.5);}
</style>
